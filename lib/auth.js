...
function resolveGrantedScopes(response, requestedScope) {
  const scopes = new Set(grantedScopes);

  normalizeScopes(response?.scope).forEach((scope) => scopes.add(scope));
  normalizeScopes(requestedScope).forEach((scope) => scopes.add(scope));
...
function handleTokenResponse(response) {
  ...
  grantedScopes = resolveGrantedScopes(response, pendingRequest?.scope);
