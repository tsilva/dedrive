...
export async function fetchAllFiles(onProgress) {
  ...
  do {
    ...
    const ancestryInventory = pageFiles.filter((file) => {
      return file.ownedByMe || file.mimeType === FOLDER_MIME_TYPE;
    });
    ownedFileCount += pageFiles.filter((file) => file.ownedByMe).length;
    files = files.concat(ancestryInventory);
