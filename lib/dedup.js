...
const SOURCE_PATH_COLLATOR = new Intl.Collator('en', {
  numeric: true,
  sensitivity: 'base',
});

function compareStableText(first, second) {
  const firstText = String(first ?? '');
  const secondText = String(second ?? '');
  const naturalOrder = SOURCE_PATH_COLLATOR.compare(firstText, secondText);
...
function getFileSourcePath(file) {
  if (file?.path) return file.path;

  const parentNames = Array.isArray(file?.parentChain)
    ? file.parentChain.map((segment) => segment?.name ?? '')
    : [];
  return '/' + [...parentNames, file?.name ?? ''].join('/');
}

function compareFilesBySourcePath(first, second) {
  return compareStableText(getFileSourcePath(first), getFileSourcePath(second))
    || compareStableText(first?.id, second?.id);
}

function compareDuplicateGroups(first, second) {
  const sharedLength = Math.min(first.files.length, second.files.length);

  for (let index = 0; index < sharedLength; index++) {
    const fileOrder = compareFilesBySourcePath(first.files[index], second.files[index]);
    if (fileOrder !== 0) return fileOrder;
  }

  return first.files.length - second.files.length
    || compareStableText(first.md5, second.md5);
}
...
export function isPathBlacklisted(path, prefixes) {
  if (!path || !Array.isArray(prefixes) || prefixes.length === 0) return false;
...
export function excludeBlacklistedPathFiles(files, prefixes) {
  const normalizedPrefixes = [...new Set(
    (Array.isArray(prefixes) ? prefixes : []).map(normalizePathPrefix).filter(Boolean)
  )];

  return files.filter((file) => {
    const blacklisted = isPathBlacklisted(file?.path, normalizedPrefixes);
    file.pathBlacklisted = blacklisted;
    return !blacklisted;
  });
}
...
export function findDuplicates(files) {
  ...
  for (const [md5, groupFiles] of byMd5) {
    ...
    const sortedFiles = [...groupFiles].sort(compareFilesBySourcePath);
    const sizes = new Set(sortedFiles.map((f) => f.size));
    groups.push({
      md5,
      files: sortedFiles,
      uncertain: sizes.size > 1,
      totalSize: sortedFiles.reduce((sum, f) => sum + (parseInt(f.size) || 0), 0),
      wastedSize: sortedFiles.slice(1).reduce((sum, f) => sum + (parseInt(f.size) || 0), 0),
    });
  }
